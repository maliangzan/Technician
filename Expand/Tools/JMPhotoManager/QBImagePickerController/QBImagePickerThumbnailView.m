...
//    for (int i =0 ; i<self.thumbnailImages.count; i++) {
//        thumbnailImage=[self.thumbnailImages objectAtIndex:i];
//        if (thumbnailImage) {
//            break;
//        }
//    }
...
//    CGContextRef ctr=UIGraphicsGetCurrentContext();
  //  CGContextAddPath(UIGraphicsGetCurrentContext(),
    ...
    if (thumbnailImage.size.width>thumbnailImage.size.height) {
        width = width * thumbnailImage.size.width/thumbnailImage.size.height;
     
    }else
    {
        height = height * thumbnailImage.size.height/thumbnailImage.size.width;
    }
    
    CGRect thumbnailImageRect = CGRectMake(0, 4.0, width, height);
    CGContextFillRect(context, thumbnailImageRect);
    [thumbnailImage drawInRect:CGRectInset(thumbnailImageRect, 0.5, 0.5)];
}
