...
- (instancetype)initWithFrame:(CGRect)frame withBeginIndex:(NSInteger)beginIndex endIndex:(NSInteger)endIndex{
    if ([super initWithFrame:frame]) {
        [self buildUI];
        self.backgroundColor = [UIColor whiteColor];
        self.begingIndex = beginIndex;
...
- (void)drawRect:(CGRect)rect{
    [super drawRect:rect];
    [self.pickerView selectRow:self.begingIndex inComponent:0 animated:NO];
    [self.pickerView selectRow:self.endIndex inComponent:2 animated:NO];
}
