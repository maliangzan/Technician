...
- (void)viewDidLoad {
    ...
    ObserveNotification(kNotificationLoginOut, @selector(loginOut));
    [self bindData];
}
...
- (LabelSwitchCell *)gettingLabelSwichCellInTableView:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    self.voiceSwitch = cell.voiceSwitch;
    [self bindData];
    cell.delegate = self;
    if(cell==nil){
...
- (void)loginOutAction:(UIButton *)btn{
    WeakSelf;
    [[SYAlertViewTwo(Localized(@"提示"), Localized(@"确认退出登录？"), Localized(@"取消"), Localized(@"确定")) setCompleteBlock:^(UIAlertView *alertView, NSInteger index) {
        if (index == 1) {
            [weakself.navigationController popToRootViewControllerAnimated:NO];
            [[SYAppConfig shared] logout];
...
- (void)bindData {
    self.voiceSwitch.on = [[NSUserDefaults standardUserDefaults] boolForKey:@"voiceSwitch"];
    
    [self.voiceSwitch.rac_newOnChannel subscribeNext:^(id x) {
        
        [[JYAppData shared] putNotificationPlaySound:[x boolValue]];
        
        [JYNotificationHelper shared].enableSound = [x boolValue];
        [[NSUserDefaults standardUserDefaults] setBool:[x boolValue] forKey:@"voiceSwitch"];
...
- (void)rightBtnClickAtCell:(LabelButtonCell *)cell clickButton:(UIButton *)btn{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:btn.selected forKey:autoLocationStateKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
//        [cell.rightBtn setImage:PNGIMAGE(@"img_notice") forState:(UIControlStateNormal)];
        [cell.rightBtn setImage:PNGIMAGE(@"") forState:(UIControlStateNormal)];
        cell.titleLabel.text = titleStr;
    ...
    switch (indexPath.section) {
        ...
        case 2:
        {
            switch (indexPath.row) {
                ...
                {
                    //版本更新
                    [self.view showHUDForInfo:Localized(@"当前已是最新版本")];
...
-(NSArray *)titleArray{
    if (!_titleArray) {
        ...
        if ([SYAppConfig shared].me.hasLogin) {
            array3 = @[@"服务标准",@"服务条款",@"服务协议",@"版本更新",@"安全退出"];
        }else{
            array3 = @[@"服务标准",@"服务条款",@"服务协议",@"版本更新"];
        }
