...
static ALAssetsLibrary *_assetLibrary;
...
//        ALAssetsLibrary *assetsLibrary = [[ALAssetsLibrary alloc] init];
        ...
        self.assetsLibrary = _assetLibrary;
...
- (void)viewDidLoad
{
    ...
    self.title = NSLocalizedStringFromTable(Localized(@"相册"), @"QBImagePickerController", nil);
}
...
//        [self.navigationItem setLeftBarButtonItem:cancelButton animated:NO];
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:Localized(@"取消") style:UIBarButtonItemStylePlain target:self action:@selector(cancel:)];
        //SYBarItem(Localized(@"取消"), self, @selector(cancel:));
    } else {
...
#pragma 加了这个判断就可以不选退出来了
    if (firstOneHere) {

  
    [self.delegate imagePickerController:self didSelectAssets:nil];
  }
}
...
{
        [self.navigationController pushViewController:[self getAlbumsDetailVC:indexPath]  animated:YES];
...
-(void)jumpIndexPathAlbum:(NSIndexPath*)indexPath fromController:(UIViewController*)controller andNavigation:(UINavigationController*)navigationController{
    ...
    [self loadAssetsGroupsWithTypes:self.groupTypes
                         completion:^(NSArray *assetsGroups) {
                             ...
                             [navigationController pushViewController:[weakSelf getAlbumsDetailVC:indexPath] animated:NO];
                             [controller presentViewController:navigationController animated:YES completion:nil];
                         }];
}


-(QBAssetsCollectionViewController*)getAlbumsDetailVC:(NSIndexPath*)indexPath{
    // Load assets groups
    ...
    assetsCollectionViewController.selectedAssetURLs=self.selectedAssetURLs;
    ...
    assetsCollectionViewController.isForChat = self.isForChat;
    ...
    return assetsCollectionViewController;
